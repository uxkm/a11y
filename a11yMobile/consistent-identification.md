...
- **사용자 경험 개선**: 일관된 UI는 사용자가 애플리케이션을 보다 직관적이고 쉽게 사용할 수 있도록 합니다.    
- **학습 용이성**: 사용자는 비슷한 패턴을 반복적으로 사용하면 앱 내에서 새로운 기능을 더 빠르게 익힐 수 있습니다.    
- **접근성 향상**: 일관된 인터페이스는 시각 장애인 및 인지 장애를 가진 사용자가 쉽게 이해하고 탐색할 수 있도록 도와줍니다.    

저시력 인이나 고령자 등 화면을 확대하는 사용자의 경우에는 전체 화면이 아니라 창의 일부 영역을화면에 확대하여 이용한다. 따라서 애플리케이션 창마다 내비게이션 컨트롤의 위치와 모양이 다르다면 새로운 애플리케이션 창으로 이동할 때마다 사용법을 익히는 데 많은 어려움이 있을 것이다. 또한 지적 장애인의 경우 이용한 애플리케이션 별로 메뉴와 내비게이션 컨트롤의 위치나 모양이 바뀌게 되면 사용자는 페이지를 동일한 애플리케이션이 제공하는 페이지로 인식하기보다는 새로운 애플리케이션이 제공하는 페이지로 인식할 가능성이 높다. 이에 일관성 있는 사용자 인터페이스를 제공하는 것이 바람직하다. 

#### 2. 대상       

- **모든 사용자**: 시각적, 인지적 이해를 돕기 위해 일관된 UI는 모든 사용자에게 유리합니다.    
- **장애인 사용자**: 특히 시각 장애인, 인지 장애가 있는 사용자에게는 인터페이스의 일관성이 중요한 탐색 요소입니다.    
- **고령 사용자**: 기억력이 떨어지거나 복잡한 UI를 다루기 어려운 고령 사용자에게 큰 도움이 됩니다.    

#### 3. 체크리스트      

- **일관된 디자인 시스템 사용**: 앱의 전반적인 UI 디자인에서 동일한 색상, 글꼴, 버튼 스타일을 유지합니다.    
- **컴포넌트 위치의 일관성**: 중요한 UI 요소(예: 탐색 메뉴, 버튼 등)는 앱의 다른 화면에서도 동일한 위치에 유지됩니다.    
- **유사한 상호작용 패턴**: 같은 유형의 기능이 동일한 상호작용 방식을 사용하도록 보장합니다.   
- **동일한 아이콘 사용**: 특정 작업에 대해 동일한 아이콘이 사용되어야 합니다.   

#### 4. 기기별 테스트 방법      

- **iOS**: 다양한 기기 크기(iPhone, iPad)에서 UI 요소의 배치와 일관성을 확인합니다.   
- **Android**: 다양한 화면 해상도와 OS 버전에서 UI 요소가 일관되게 표시되는지 점검합니다.   
- **웹 및 하이브리드 앱(HTML, Vue, React)**: 여러 브라우저와 화면 크기에서 UI 컴포넌트의 배치가 일관되게 유지되는지 테스트합니다.   

#### 5. QA 지표       

- **UI 일관성 비율**: 각 화면에서 컴포넌트의 일관성을 측정하여 일관성이 유지되는지 확인합니다.    
- **사용자 피드백**: 사용자 테스트를 통해 UI의 일관성에 대한 의견을 수집합니다.   
- **시각적 점검**: UI 디자인의 가이드라인 준수 여부를 시각적으로 확인합니다.    

#### 6. 개발방법     

- **디자인 시스템 도입**: Figma, Sketch와 같은 디자인 도구에서 일관된 UI 요소를 정의하고 사용합니다.    
- **CSS 클래스 및 공통 스타일 사용**: 동일한 CSS 클래스나 컴포넌트 라이브러리를 사용하여 스타일의 일관성을 보장합니다.   
...
- **디자인 가이드라인 준수 여부**: UI 요소가 사전 정의된 디자인 가이드라인에 맞게 구현되었는지 확인합니다.   
- **반응형 디자인 확인**: 화면 크기에 따른 UI의 일관성을 점검합니다.   
- **사용자 피드백 수집**: 다양한 사용자 그룹의 피드백을 통해 UI의 일관성 및 만족도를 평가합니다.   

#### 8. 점검 방법     

- **자동화 도구**: Storybook이나 Percy와 같은 UI 테스트 도구를 사용하여 화면의 일관성을 점검합니다.   
- **수동 점검**: 다양한 기기 및 해상도에서 시각적으로 UI의 일관성을 확인합니다.   
- **디자인 리뷰**: 디자이너와 협력하여 디자인 사양과의 일치 여부를 검토합니다.    

#### 9. 준수 사례       

- **일관된 내비게이션 패턴**: 내비게이션 바가 모든 화면에서 동일한 위치에 있고, 동일한 스타일을 유지하는 앱.    
- **유사한 인터랙션**: 같은 작업에 대해 동일한 버튼 스타일과 상호작용을 사용하는 앱.    

#### 10. 미준수 사례       

- **불일치한 버튼 스타일**: 화면마다 다른 스타일의 버튼을 사용하는 경우.    
- **위치 변경**: 주요 UI 요소(예: 메뉴 버튼)가 화면에 따라 다른 위치에 있는 경우.    
- **비일관적인 아이콘 사용**: 동일한 기능에 대해 서로 다른 아이콘을 사용하는 경우.    

#### 11. 관련 영상       
<iframe style="width:100%;min-height:315px;" src="https://www.youtube.com/embed/KgfGEzy1PUc?si=H4RhuYsB5BQabrad" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

[NULI 모바일 앱 접근성 (13. 사용자 인터페이스의 일괄성)](https://www.youtube.com/embed/KgfGEzy1PUc?si=H4RhuYsB5BQabrad){: target="_blank"}    
